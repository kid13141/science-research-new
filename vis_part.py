...
# algs = ['MAIC', 'MASIA', 'NDQ', 'QMIX', 'DTC']#, 'mixrts', 'qnam'
# algs = ["QMIX", 'TarMAC','NDQ','VBC', 'TMC', 'SMS', 'MASIA', 'MAIC','CACOM','DTC']
algs = ["qmix","maser","diff_a05","diff_a02"]
q_tree_depth = 3
mix_q_tree_depth = 3
beta = 0
...
def get_num(map):
    ...
    if smac_map == '2s3z':
        load_num = 150
    if smac_map == '3s5z_vs_3s6z' or smac_map == 'corridor':
        load_num = 300
    return load_num
nums = 3
...
def read_file(smac_map, load_num):
    ...
    for alg_name in algs:
        ...
        for num in range(1, nums+1):
            file_path = 'results/sacred_vis/'+smac_map+'/'+alg_name+'/'+str(num)+'/info.json'
            print(file_path)
...
def plt_term_mean(smac_map, load_num, files, i, j):
    ...
    for alg_name in algs:
        ...
        if alg_name == 'qmix':
            label = 'QMIX'
        elif alg_name == 'diff_a05':
            label = 'diff_a05'
        elif alg_name == 'diff_pre':
            label = 'diff_pre'
        elif alg_name == 'diff':
            label = 'diff'
        elif alg_name == "diff_a03":
            label = "diff_a03"
        elif alg_name == "diff_a07":
            label = "diff_a07"
        elif alg_name == "diff_a02":
            label = "diff_a02"
        elif alg_name == "maser":
            label = "maser"
    ...
    if load_num==50:
        plt.xticks((range(0, len(coll[alg_name]) + 3, 10)), ("0", "0.10", "0.30",  "0.50"))
    if load_num==200:
...
if __name__ == '__main__':
    ...
    # smac_maps = ['1o_2r_vs_4r', '1o_10b_vs_1r', '5z_vs_1ul','3c_vs_100zg','MMM2','6h_vs_8z','1c3s5z_vs_1c3s6z']
    # smac_maps = ['1o_2r_vs_4r', '1o_10b_vs_1r', '5z_vs_1ul','6h_vs_8z','3c_vs_100zg','5m_vs_5m','1c4z_vs_1ul_v1','2s3z_vs_1ul_v1']
    # smac_maps = ['3c_vs_100zg','6h_vs_8z','corridor', '1o_2r_vs_4r', '1o_10b_vs_1r', '5z_vs_1ul','5m_large','1c4z_vs_1ul','2s3z_vs_1ul']
    # smac_maps = ['1o_2r_vs_4r', '1o_10b_vs_1r', '5z_vs_1ul','6h_vs_8z']
    smac_maps = ['5m_vs_6m_v2']
    ax = plt.figure(figsize=(10, 9), dpi=400)
    ...
    plt.legend(handles, labels, ncol=2, bbox_to_anchor=(0.5, 0.9))
    plt.savefig('./results_part.pdf',bbox_inches='tight')
    # plt.show()