...
class DiffLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Diffusion Training
        if not self.args.load_diffusion:
            diffusion_losses = []
            ...
            diffusion_loss = th.zeros(1)
            diff_return = th.ones(1)
        ...
        hidden_states =[] #(201,160,64)
        for t in range(max_seq_length):
            agent_outs,hidden_states_outs = self.mac.forward(batch, t=t, goals=split_goals_org) 
            mac_out.append(agent_outs)
            # hidden_states_outs = hidden_states_outs.tolist()
            hidden_states.append(hidden_states_outs)

        mac_out = th.stack(mac_out, dim=1)  # Concat over time
        # hidden_states = th.tensor(hidden_states) # (201,160,64)
        hidden_states = th.stack(hidden_states, dim=0) #(160,201,64)
        ...
        hidden_states = stacked_tensor.permute(1, 0, 2, 3).cuda() #(32,160,5,64)
        ...
        for t in range(max_seq_length):
            target_agent_outs,_= self.target_mac.forward(batch, t=t, goals = split_goals_org)
        ...
        factor_rewards = np.array(factor_rewards.squeeze().cpu())
    ...
    def _update_targets_soft(self,tau):
        # self.target_mac.load_state(self.mac)
        # self.target_IAU.load_state_dict(self.IAU.state_dict())
        for target_param, param in zip(self.target_IAU.parameters(), self.IAU.parameters()):
    ...
    def rebuild_diff_batch(self, batch, good_batch=False):
        ...
        for i in range(state.shape[0]):
            ...
            for j in range(self.args.sample_size):
                discount_return = 0
                reward_array = rewards[i][goal_index[j]:avil_state_range2].cpu().numpy().squeeze()
