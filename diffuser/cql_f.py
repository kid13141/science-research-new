...
class ReplayBuffer:
    ...
    def add_transition(self, obs, next_obs, actions, rewards, terminals):
        batch_size = len(obs)
        if self._pointer + batch_size <= self._buffer_size:
            begin = self._pointer
            end = self._pointer + batch_size
            self._states[begin:end] = self._to_tensor(obs)
            self._actions[begin:end] = self._to_tensor(actions)
            self._rewards[begin:end] = self._to_tensor(rewards)
            self._next_states[begin:end] = self._to_tensor(next_obs)
            self._dones[begin:end] = self._to_tensor(terminals)
            ...
            self._states[begin:end] = self._to_tensor(obs[:first_add_size])
            self._actions[begin:end] = self._to_tensor(actions[:first_add_size])
            self._rewards[begin:end] = self._to_tensor(rewards[:first_add_size])
            self._next_states[begin:end] = self._to_tensor(next_obs[:first_add_size])
            self._dones[begin:end] = self._to_tensor(terminals[:first_add_size])
...
class ContinuousCQL:
    ...
    def _v_loss(
        self, observations, actions, log_dict
    ):
        with torch.no_grad():
            q1_predicted = self.critic_1(observations, actions)
            q2_predicted = self.critic_2(observations, actions)
        ...
        adv = torch.min(q1_predicted, q2_predicted) - v
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        v_loss = self._v_loss(observations, actions, log_dict)
...
def env_rollout(forward_dynamics_model, backward_dynamics_model, back_policy_model, generator,reward_network, replay_buffer, forward_model_buffer, backward_model_buffer, trainer, config):
    
    except_task = ["pen","door","relocate","hammer"]
    task = config.env.split('-')[0]
    if task in except_task:
            import_path = f"static_fns.except"
    else:
    ...
    with torch.no_grad():
        for i in range(config.rollout_length):
            print("step: ", i+1)
            new_actions = trainer.actor.act(observations, config.device)
            new_actions = new_actions.reshape(observations.shape[0], actions.shape[1])
            inputs = torch.cat((observations, torchify(new_actions)), 1)
            rewards = reward_network.forward(inputs)

            ensemble_next_obs = np.zeros((config.ensemble_size, observations.shape[0], observations.shape[1]))
            for j in range(config.ensemble_size):
                next_obs_mem = forward_dynamics_model.forward(inputs, True)
                # next_obs_mem = self.model.guided_sample(inputs, self.policy.value_net)[:,:obs.shape[1]]
                ensemble_next_obs[j] = next_obs_mem.cpu().detach().numpy()
            
            batch_inds = np.arange(0, observations.shape[0])
            return_inds = np.random.choice(config.ensemble_size, size=observations.shape[0])
            next_obs = ensemble_next_obs[return_inds, batch_inds]

            mean_obs_means = np.mean(ensemble_next_obs, axis=0)
            diffs = ensemble_next_obs - mean_obs_means
            dists = np.linalg.norm(diffs, axis=2)   # distance in obs space
            penalty = np.max(dists, axis=0)         # max distances over models
            print("uncertainty_mean:",penalty.mean())

            terminals = static_fns.termination_fn(observations, new_actions, next_obs)

            rewards = rewards.cpu().detach().numpy()
            penalty = np.expand_dims(penalty,axis=1)
            print("unpenalized_rewards_mean:", np.mean(rewards))

            rewards = rewards - penalty
            print("penalized_rewards_mean:", np.mean(rewards))

            forward_model_buffer.add_transition(observations, torchify(next_obs) , new_actions, torchify(rewards), torchify(terminals))
            terminals_index = []
            for j in range(observations.shape[0]):
                if terminals[j][0] == 0:
                    terminals_index.append(j)
            print("current state num: ", len(terminals_index))
            if len(terminals_index) == 0:
                break
            terminals_index = np.array(terminals_index)
            next_obs = next_obs[terminals_index]
            observations = torchify(next_obs)


def data_sample(replay_buffer, model_buffer, config):
    real_data_size = int(config.batch_size * config.real_ratio)
    fake_data_size = config.batch_size - real_data_size
    real_data = replay_buffer.sample(real_data_size)
    fake_data = model_buffer.sample(fake_data_size)
    for i in range(len(real_data)):
        real_data[i] = torch.cat((real_data[i],fake_data[i]), 0)
    return real_data
...
def train(config: TrainConfig):
    ...
    backward_model_buffer = ReplayBuffer(
        state_dim,
        action_dim,
        config.rollout_length * config.sample_size,
        config.device,
    )
    forward_model_buffer = ReplayBuffer(
        state_dim,
        action_dim,
        config.rollout_length * config.sample_size * 5,
        config.device,
    )
    ...
    # run_name = config.env + "_" + str(config.seed) + "_" + str(config.rollout_length) + "_" + str(config.lambda_guide) + "_" + str(config.real_ratio) + "_" + str(config.cql_min_q_weight)
    # wandb.init(settings=wandb.Settings(start_method='thread'),project="CORL-CQL-FORWARD-BACKWARD", entity="ssc", name = run_name)
    # wandb.config.update(config)

    # forward_dynamics_network = Backward_Net(observation_dim = state_dim, action_dim = action_dim, input_dim = state_dim+state_dim+16+action_dim, out_dim = state_dim, t_dim = 16, hidden_dim=256)
    # diffusion_forward_dynamics = GaussianDiffusion(forward_dynamics_network, state_dim, action_dim, state_dim, max_state, 1, config.n_diffusion_steps).to(config.device)
    # f_optimizer = torch.optim.Adam(diffusion_forward_dynamics.parameters(), lr=3e-4)

    # back_dynamics_network = Backward_Net(observation_dim = state_dim, action_dim = action_dim, input_dim = state_dim+state_dim+16+action_dim, out_dim = state_dim, t_dim = 16, hidden_dim=256)
    # diffusion_backward_dynamics = GaussianDiffusion(back_dynamics_network, state_dim, action_dim, state_dim, max_state, 1, config.n_diffusion_steps).to(config.device)
    # d_optimizer = torch.optim.Adam(diffusion_backward_dynamics.parameters(), lr=3e-4)

    # back_policy_network = Backward_Net(observation_dim = state_dim, action_dim = action_dim, input_dim = state_dim+16+action_dim, out_dim = action_dim, t_dim = 16, hidden_dim=256)
    # diffusion_policy = GaussianDiffusion(back_policy_network, state_dim, action_dim, action_dim, max_action, 1,config.n_diffusion_steps).to(config.device)
    # p_optimizer = torch.optim.Adam(diffusion_policy.parameters(), lr=3e-4)
            
    #train diffusion
    back_generator_network = Backward_Net(observation_dim = state_dim, action_dim = action_dim, input_dim = state_dim+16, out_dim = state_dim, t_dim = 16, hidden_dim=256)
    diffusion_generator = GaussianDiffusion(back_generator_network, state_dim, action_dim, state_dim, max_state, config.lambda_guide, 100).to(config.device)
    ...
    for t in range(config.train_step):
        ...
        # f_loss = diffusion_forward_dynamics.loss(x = next_observations, cond = torch.cat((observations, actions), 1), is_cond = True)
        # f_loss.backward()
        # f_optimizer.step()
        # f_optimizer.zero_grad()

        # d_loss = diffusion_backward_dynamics.loss(x = observations, cond = torch.cat((next_observations, actions), 1), is_cond = True)
        # d_loss.backward()
        # d_optimizer.step()
        # d_optimizer.zero_grad()

        # p_loss = diffusion_policy.loss(x = actions, cond = next_observations, is_cond = True)
        # p_loss.backward()
        # p_optimizer.step()
        # p_optimizer.zero_grad()

        g_loss = diffusion_generator.loss(x = states, cond = None, is_cond = False)
        g_loss.backward()
        g_optimizer.step()
        g_optimizer.zero_grad()

        r = reward_network.forward(torch.cat((observations, actions), 1))
        r_loss = F.mse_loss(r, rewards)
        r_loss.backward()
        r_optimizer.step()
        ...
        wandb.log({
            "forward_dynamics_loss": f_loss.item(),
            # "backward_dynamics_loss": d_loss.item(),
            # "backward_policy_loss": p_loss.item(),
    ...
    for t in range(int(config.max_timesteps)):
        if (t+1) % config.generation_freq == 0:
            env_rollout(diffusion_forward_dynamics, diffusion_backward_dynamics, diffusion_policy, diffusion_generator,reward_network,replay_buffer, forward_model_buffer, backward_model_buffer, trainer, config)
        
        if forward_model_buffer._size < config.batch_size :
            batch = replay_buffer.sample(config.batch_size)
        else:
            batch = data_sample(replay_buffer, forward_model_buffer, config)
        batch = [b.to(config.device) for b in batch]
        log_dict = trainer.train(batch)
        # if backward_model_buffer._size >= config.batch_size:
        #     imi_batch = backward_model_buffer.sample(config.batch_size)
        #     imi_batch = [b.to(config.device) for b in imi_batch]
        #     (
        #         observations,
        #         actions,
        #         rewards,
        ...
        #     new_actions, log_probs = trainer.actor(observations)
        #     bc_losses = torch.sum((new_actions - actions) ** 2, dim=1).mean()
        #     trainer.actor_optimizer.zero_grad()
        #     bc_losses.backward()
        #     trainer.actor_optimizer.step()
        #     wandb.log({
        #         "bc loss": bc_losses.item()
        #     })
        wandb.log(log_dict)
