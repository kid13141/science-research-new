...
class Diff_Hilp3_MAC:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False,hilp_val=None,lock=None):
        # Only select actions for the selected batch elements in bs
        avail_actions = ep_batch["avail_actions"][:, t_ep]
        agent_outputs, agent_outputs_nav,_ = self.forward(ep_batch, t_ep, test_mode=test_mode)
        chosen_actions = self.action_selector.select_action(agent_outputs[bs], avail_actions[bs], t_env, test_mode=test_mode)
        chosen_actions_nav = self.action_selector.select_action(agent_outputs_nav[bs], avail_actions[bs], t_env, test_mode=test_mode)

        if test_mode:
            return chosen_actions,lock

        hilp_val_reshaped = hilp_val.view(-1, 1) 
        
        # 计算当前步的硬门控 (这里确保 self.gating_delta 已经在 MAC 的 __init__ 中定义过了)
        hard_alpha = (hilp_val_reshaped <= getattr(self, "gating_delta", 0.5)).float()
        hard_alpha = hard_alpha.view_as(lock)
        lock = th.max(lock.to(self.device), hard_alpha).detach() # 必须 detach 断开梯度
        
        # ==== 5. 张量级别的动作融合 ====
        # 如果 lock 是 1 (战斗)，采用 chosen_actions (Q_act选的)
        # 如果 lock 是 0 (赶路)，采用 nav_chosen_actions (Q_nav选的)
        final_actions = th.where(lock == 1.0, chosen_actions, chosen_actions_nav)
   
        return final_actions, lock

    def forward(self,ep_batch, t,test_mode=False):
        agent_inputs = self._build_inputs(ep_batch, t)
        avail_actions = ep_batch["avail_actions"][:, t]
        agent_outs,agent_outs_nav,self.hidden_states = self.agent(agent_inputs , self.hidden_states)
        ...
        return agent_outs.view(ep_batch["state"].shape[0], self.n_agents, -1), agent_outs_nav.view(ep_batch["state"].shape[0], self.n_agents, -1),self.hidden_states.view(ep_batch["state"].shape[0], self.n_agents, -1).detach()
