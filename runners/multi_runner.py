...
class MultiRunner:
    ...
    def compute_post_influence(self, state, next_state, total_reward, ally_alive, enemy_alive):
        influences = []
        ...
        for i in range(self.args.n_agents + self.args.n_enemy):
            ...
            org_state = next_state.clone()
            next_state[0, start_index:end_index] = state[0, start_index:end_index]
            mod_state = next_state

            mod_dist = phi_distance(self.mac, mod_state, org_state)

            influence = mod_dist * alive
            influences.append(influence.item())
        total_influence = sum(influences)
        # 如果总影响力为0，则可以设置默认的均匀权重
        if total_influence == 0:
            normalized_weights = [1 / len(influences)] * len(influences)
        else:
            # 计算每个智能体的权重（正负由影响力决定）
            normalized_weights = [(influence / total_influence)  for influence in influences]

        exp_reward = total_reward.cpu().numpy() * np.array(normalized_weights)
        exp_reward = exp_reward[:self.args.n_agents]
        return exp_reward, exp_reward.sum().item()
        # total_influence = sum(influences)
        # bias = (total_reward - total_influence)/(self.args.n_agents + self.args.n_enemy)
    ...
    def run(self, test_mode=False, diff_return=1.0, v_thed=1.0):
        ...
        while not terminated:
            if self.t == 0:
                ...
                norm_state_th = to_tensor(norm_state)

                returns = torch.ones(norm_state_th.shape[0], 1).cuda() * diff_return
                conditions = {0: norm_state_th}

                with torch.no_grad():
                    goal_states_th = self.mac.diffusion_agent.forward(cond=conditions, returns=returns) # sample (s_0, g) from diffusion model
                    goal_states = non_normalized(to_numpy(goal_states_th), self.args.states_max, self.args.states_min)
                    goal_states = to_tensor(goal_states)
            ...
            pre_transition_data = {
                ...
                "multi_goals": [to_numpy(goal_states)],
            ...
            # factor_reward = np.clip(factor_reward, 0, None)
