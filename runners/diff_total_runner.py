        ...
        while not terminated:
            if self.t == 0:
                ...
                if self.args.swap_goals and random.random() < 0.1 and not test_mode:
                    ...
                    goal_states[:, :self.args.n_agents * self.args.teammate_dim] = split_goals.view(goal_states.shape[0], -1)
            ...
            total_dis = cur_dis.sum()
            if total_dis < min_dis:
                min_dis = total_dis
                t = self.t

            if terminated and self.t_env > 5e4:
                total_reward = (init_dis - min_dis)* t / 200
            else:
                total_reward = 0
